...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for MongoDB-Atlas").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-mongodb-atlas"))
	if *debug {
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add MongoDB-Atlas APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup MongoDB-Atlas controllers")
